...
describe('AdminErrorPage', () => {
  ...
  it('renders an error message, logs it, and offers retry and dashboard actions', async () => {
    const reset = jest.fn();
  ...
  it('shows the API status code when the error came from a thrown query error', () => {
    ...
    renderWithIntl(<AdminErrorPage error={apiError} reset={jest.fn()} />);
  ...
  it('omits the error code line when the error has neither status nor digest', () => {
    const { container } = renderWithIntl(
      <AdminErrorPage error={new Error('boom')} reset={jest.fn()} />,
    );
  ...
  it('shows an access-denied message with no retry button for a 403', () => {
    ...
    renderWithIntl(<AdminErrorPage error={forbiddenError} reset={jest.fn()} />);
